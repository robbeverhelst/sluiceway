...
| `core/` | Pure logic: types, config, which discovered stacks exist (`ignore`, one id per stack), the diff hash, the pool a scan previews through, when a scan could not do its work, the claim rule, the tick rule, attribution. | No |
...
| `github/` | Everything that talks to GitHub or to the runner: the port and its Octokit implementation, the write loop, the dashboard, the action ref, the inputs, the facts of the job, and the job log with its annotations and summary. | Yes |
| `render/` | Turns scan results into the dashboard body: rows, markers, the body around the rows with its header state and its voice, the size budget. Also the summary of a scan and the log text of a diff. | No |
