...
// reports, and without this its stack would stay deploying. It is handed no
...
export async function settle(context: SettleContext): Promise<void> {
  ...
  for (const { id, stackId: stack } of open) {
    try {
...
async function startedHere(
  ...
  const hinted = new Set(
    rows.flatMap((row) =>
      row.known && (row.state === "deploying" || row.ticked) ? [row.stackId] : [],
    ),
  );
