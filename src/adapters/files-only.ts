...
// (record 0074), and what its files say the tool will want from the job
// environment (record 0099). Nothing here starts a tool (record 0042), so the
// check job and the command line (record 0094) hand the check the same thing.
export const filesOnly: Pick<
  Adapter,
  "discover" | "readsFiles" | "explainDiscovery" | "credentialNeeds"
> = {
