...
| 12 | The first dashboard of a real repo showed the plain header: no colour, no mascot, no voice. Four pending rows held a replace or a delete, and three of those were routine replacements of a Kubernetes Secret or ConfigMap, which Pulumi replaces whenever their content changes. In a Kubernetes repo that is the normal state, so the header that sets Sluiceway apart would almost never show. | "Where is the thing from the screenshots?" | **open, for the owner.** The rule (0031, 0032: any delete or replace on a pending or deploying row turns the header plain) was made for safety and stays right for a deleted database. The core cannot tell a ConfigMap from a database. Options: keep it; go plain only for deletes and keep the row warning for replaces; or replace the plain header with an in-brand "careful" state that has no playful voice. |
