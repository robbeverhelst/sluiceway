...
- `scan` now sets `matrix` too, always `[]` unless it handed a merge on. The workflow's `apply` and `settle` jobs take `needs.resolve.outputs.matrix || needs.scan.outputs.matrix`. The README's default workflow is unchanged, because the feature is off by default, and its section on merge and deploy shows the three changes.
