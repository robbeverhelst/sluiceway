...
- The deploy may have a time limit of Sluiceway's: the `deploy-timeout` input of `apply`, in whole minutes, empty by default. The worry that kept it out still holds, so it is the workflow's choice and off unless set: stopping a deploy half way can leave a stack half deployed. With it, every run of the tool in the deploy gets the limit, and when one runs out the whole process group gets SIGINT and two minutes to stop by itself before SIGKILL, where a preview gets five seconds: an interrupted Pulumi or OpenTofu finishes what it is doing to a resource and lets go of the state's lock. The record ends as `failure` with `the deploy ran out of its time limit of N minutes and the tool was stopped`, and the row comes from a preview of what is left, as after any failed deploy. The mode wraps the runner it hands the adapter, so no adapter changed: each reads a run that timed out as a tool error without an exit code, and the mode, which knows the limit ran out, gives the reason.
