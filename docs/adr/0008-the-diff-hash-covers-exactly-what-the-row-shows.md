...
>
> Amended by 0111: a merge after the checkout of the run never reaches the fresh preview, so `apply` compares the checked-out commit with the head of the branch first, and a newer commit the stack claims is refused as moved. The commit still stays out of the hash.
