...
- The stack id is `path`, or `path:name` when the stack has a name. The path is relative to the repo root, with forward slashes, no leading `./` and no trailing slash. For Pulumi the name is always present and is the Pulumi stack name, so ids look like `apps/grafana:prod`. A later OpenTofu directory per environment is `envs/prod`. A stack that lives at the repo root has the path `.`, so its id is `.:prod`. The rule has no special case for it, `path: .` in config means the same thing, and an empty path would give ids such as `:prod` or a bare `prod` that reads like a directory.
...
- A `stacks` entry may give the one stack it covers an id of its own with `id`, in place of the derived one. The worry that kept it out, a second source of identity to keep unique, is met at load: the entry must cover exactly one stack, the id is plain (letters, digits and `. _ / : @ + -`), and an id that another stack has, derived or given, is refused. It is applied right after discovery, so `ignore`, `dependsOn`, `dependsOn: auto`, the rows, the deployment tasks and the check all see the given id, and an entry matches the stack by its path and name as before. The use is a stack that moved: at its new path it keeps its row, its records and its trail under the id it had, where a derived id would make it a new stack.
