...
- If `refresh` is added later it is a named option that puts the flag on all three calls, so the preview and the deploy run with the same flags and the hash still matches. Rows of stacks that turn it on get a new hash once. Nothing else breaks.
