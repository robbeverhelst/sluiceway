...
| A failure line on a row that a narrowed scan carries through | A carried row is never read inside, so it keeps the failure line it has, or has not. `apply` and `settle` write that row themselves, and the next preview of the stack brings it in line. | 0004, slice 2.1 |
...
| A cap above 10 KB per header file | The fullest files are at 9.9 KB. Final art that needs more asks the owner, with a reason. | 0039 |
...
| Final art for Penny | The files in `assets/mascot/` are generated concept art after one polish round, good enough to ship a first release with. Final art may redraw every shape and keeps the character, the colour pair, the composition, the states, the pending levels and the file rules. | 0030, 0033, 0038, 0039 |
