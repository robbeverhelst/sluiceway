...
To turn it into the whole workflow later, replace the file with [the whole workflow](workflow.md#the-workflow), change `actions: read` to `actions: write`, and take `readOnly: true` out of `sluiceway.yaml`. The change to `sluiceway.yaml` makes the next push a full scan, and every pending row gets its box back.
